...
void simulate_general_hawkes(const double mu[], const double rho[], const double m[], const int M[], const double epsilon[], const double n[], 
	double T, int dim, int max, double * process_list, int * process_track, unsigned int seed, double(*func)(int loc, double t))
{
	srand(seed);
	double s{ 0.0 };
	int l{ 0 };
				...
				if (process_track[j] >= 1)
				{
					...
					else
					{
						for (int k = 0; k != M[i * dim + j]; ++k)
						{
							...
							{
								upper_lambda += (n[i * dim + j] / Z[i * dim + j]) * pow(alpha[max_M * (i * dim + j) + k], 
									-(1 + epsilon[i * dim + j])) * exp(-(s - process_list[l]) / alpha[max_M * (i * dim + j) + k]);
							}
						}
					}
				}
			...
			for (int j = 0; j != dim; ++j)
			{
				if (process_track[j] >= 1)
				{
					if (process_track[i] >= 1)
					{
						for (int k = 0; k != M[i * dim + j]; ++k)
						{
							individual_lambda[i] += (n[i * dim + j] / Z[i * dim + j]) * pow(alpha[max_M * (i * dim + j) + k], 
								-(1 + epsilon[i * dim + j])) * (exp(-(s - process_list[max * i + process_track[i] - 1]) / 
									alpha[max_M * (i * dim + j) + k]) * recursive_array[max_M * (i * dim + j) + k] + 
									exp(- candidate / alpha[max_M * (i * dim + j) + k]) * other_process[max_M * (i * dim + j) + k]);
						}
					}
					else
					{
			...
			for (int i = 0; i != dim; ++i)
			{
				if (i != l)
				{
					for (int j = 0; j != M[l * dim + i]; ++j)
					{
						...
						{
							recursive_array[max_M * (l * dim + i) + j] = exp(-(s - process_list[max * l + process_track[l] - 2]) /
								alpha[max_M * (l * dim + i) + j]) * recursive_array[max_M * (l * dim + i) + j] +
								exp(-candidate / alpha[max_M * (l * dim + i) + j]) * other_process[max_M * (l * dim + i) + j];
						}
				...
				else
				{
					if (process_track[l] >= 2)
					{
						...
						{
							recursive_array[max_M * (l * dim + i) + k] = exp(-(s - process_list[max * l + process_track[l] - 2]) /
								alpha[max_M * (l * dim + i) + k]) * (1.0 + recursive_array[max_M * (l * dim + i) + k]);
						}
					}
				}
...
void compensator_general_hawkes(const double mu[], const double rho[], const double m[], const int M[], const double epsilon[], const double n[], 
	...
	double(*func)(int loc, double t1, double t2))
{
	...
	// Shall be valid if passed by a loglikelihood_power_hawkes
	if (Z == 0 || alpha == 0)
	{
		// Creating a matrix of alpha = (rho * m) ** i and calculating Z values
	...
	{
		if (i != 0)
			compensator = func(pos, process_list[max * pos + i], process_list[max * pos + i - 1]); 
		else
			compensator = func(pos, process_list[max * pos + i], 0); 
				...
				if (i != 0)
				{
					for (int k = 0; k != M[pos * dim + j]; ++k)
					{
						compensator += (n[pos * dim + j] / Z[j]) * pow(alpha[max_M * j + k], -epsilon[pos * dim + j]) *
							(1 - exp(-(process_list[pos * max + i] - process_list[max * pos + i - 1]) /
								alpha[max_M * j + k])) * (recursive_array[max_M * j + k] + 1);
						if (i >= 1)
						{
							recursive_array[max_M * j + k] = (1 + recursive_array[max_M * j + k]) * 
								exp(-(process_list[pos * max + i] - process_list[max * pos + i - 1]) / alpha[max_M * j + k]);
						}
					}
				}
			}
			else
			{
				...
				else
				{
					for (int k = 0; k != M[pos * dim + j]; ++k)
					{
						...
						compensator += (n[pos * dim + j] / Z[j]) * pow(alpha[max_M * j + k], -epsilon[pos * dim + j]) *
							(((1 - exp(-(process_list[pos * max + i] - process_list[pos * max + i - 1]) / 
								alpha[max_M * j + k])) * recursive_array[max_M * j + k]) + counter - other_processes);
						recursive_array[max_M * j + k] = exp(- (process_list[pos * max + i] - process_list[pos * max + i - 1]) / 
							alpha[max_M * j + k]) * recursive_array[max_M * j + k] + other_processes;
					}
					other_process_track[j] = index;
				}
			}
...
double loglikelihood_general_hawkes(double mu[], double rho[], double m[], int M[], double epsilon[], double n[], double T, int pos, int dim, int max,
	double * process_list, int * process_track, double(*func)(int loc, double t), double(*func_int)(int loc, double t1, double t2))
{
	...
	compensator_general_hawkes(mu, rho, m, M, epsilon, n, T, pos, dim, max, process_list, process_track, compensator_series, 
		Z, alpha, func_int);
	...
	for (int i = 0; i != process_track[pos]; ++i)
	{
		...
		for (int j = 0; j != dim; ++j)
		{
			if (j == pos && i >= 1)
			{
				for (int k = 0; k != M[pos * dim + j]; ++k)
				{
					...
					recursive_sum += recursive_array[max_M * j + k] * (n[pos * dim + j] / Z[j]) * pow(alpha[max_M * j + k], 
						-(1 + epsilon[pos * dim + j]));
				}
			}
			else
			{
				if (i != 0)
				{
					for (int k = 0; k != M[pos * dim + j]; ++k)
					{
						...
						{
							other_process += exp(-(process_list[max * pos + i] - process_list[max * j + index]) / alpha[max_M * j + k]);
							index += 1;
							if (index == process_track[j])
							{
								index -= 1;
		...
		intensity = func(pos, process_list[max * pos + i]);
		loglikelihood += log(intensity + recursive_sum);
	}
	...
	plt_list, int * plt_end, double(*func)(int loc, double t))
{
	...
	if (s >= begin)
	{
		for (int i = 0; i != dim; ++i)
		{
			plt_intensity[i * plt_length + plt_end[i]] = func(i, s);
	...
	while (s - 0.00001  <= end - grid)
	{
		...
		for (holder h : ordering_vec)
		{
			int i = h.index;
			if (s >= begin)
				plt_intensity[i * plt_length + plt_end[i]] = func(i, h.stamp);
			int_track[i] += 1;
			...
			for (int j = 0; j != dim; ++j)
			{
				if (i != j)
				{
					for (int k = 0; k != M[i * dim + j]; ++k)
					{
						...
						{
							other_process[max_M * (i * dim + j) + k] += exp(-(process_list[max * i + int_track[i] - 1] -
								process_list[l]) / alpha[max_M * (i * dim + j) + k]);
						}
						if (int_track[i] >= 2)
						{
				...
				if (s >= begin)
				{
					...
					else if (int_track[j] >= 1 && i == j)
					{
						...
						{
							plt_intensity[i * plt_length + plt_end[i]] += (n[i * dim + j] / Z[i * dim + j]) *
								pow(alpha[max_M * (i * dim + j) + k], -(1 + epsilon[i * dim + j])) * (recursive_array[max_M * (i * dim + j) + k] + 1.0);
						}
					}
				}
			}
					...
					if (int_track[j] >= 1)
					{
						if (int_track[i] >= 1)
						{
							...
							else
							{
								for (int k = 0; k != M[i * dim + j]; ++k)
								{
									plt_intensity[i * plt_length + plt_end[i]] += (n[i * dim + j] / Z[i * dim + j]) * pow(alpha[max_M * (i * dim + j) + k],
										-(1 + epsilon[i * dim + j])) * exp(-(s - process_list[max * i + int_track[i] - 1]) / alpha[max_M * (i * dim + j) + k]) *
										(1.0 + recursive_array[max_M * (i * dim + j) + k]);
								}
							}
						}
						else
